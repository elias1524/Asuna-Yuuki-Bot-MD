...
if (!isNumber(user.corazones)) user.cookies = 20
...
if (m.corazones)
conn.reply(m.chat, `Utilizaste *${+m.corazones}* ❤️`, m, fake)
}
if (m.money)
...
if (db.data.chats[m.chat].reaction && m.text.match(/(ción|dad|aje|oso|izar|mente|pero|tion|age|ous|ate|and|but|ify|ai|genesis|megu|a|s)/gi)) {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@date', global.botdate).replace('@time', global.bottime).replace('@readMore', global.readMore).replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*Ai-Genesis*\n𝗦𝗶𝗻 𝗗𝗲𝘀𝗰𝗿𝗶𝗽𝗰𝗶𝗼𝗻') :
...
const msg = {
rowner: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡ESTA FUNCIÓN SOLO PUEDE SER USADA POR MI CREADOR!!```', 
owner: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡ESTA FUNCIÓN SOLO PUEDE SER USADA POR MI DESARROLLADOR!!```', 
mods: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡ESTA FUNCIÓN SOLO PUEDE SER USADA POR MIS DESARROLLADORES!!```', 
premium: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡ESTA FUNCIÓN SOLO ES PARA USUARIOS PREMIUM!!```', 
group: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡ESTA FUNCION SOLO PUEDE SER EJECUTADA EN GRUPOS!!```', 
private: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡ESTA FUNCIÓN SOLO PUEDE SER USADA EN CHAT PRIVADO!!```', 
admin: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡ESTE COMANDO SOLO PUEDE SER USADO POR ADMINS!!```', 
botAdmin: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡PARA USAR ESTA FUNCIÓN DEBO SER ADMIN DEL GRUPO!!```', 
unreg: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡NECESITAS ESTAR REGISTRADO(A) PARA USAR ESTE COMANDO, ESCRIBE #reg PARA REGISTRARTE!!```',
restrict: '```🌸๋๊᪷᪶᪼᪲᪻᪾᪸⃟༓༹⛦. ¡¡ESTA CARACTERÍSTICA ESTA DESACTIVADA!!```'
}[type];
