...
const handler = async (m, {conn, command, args}) => {
  ...
  if (global.db.data.users[m.sender].cookies >= xppercookies * count) {
    global.db.data.users[m.sender].corazones -= xppercorazones * count;
...
││ *Gastado* : -${xppercorazones * count} corazones ❤️
││ *Tokens Disponibles* : ${userRents[m.sender].tokens}
│╰──────────────┄
└──────────────`, m, rcanal);
  } else {
    conn.reply(m.chat, `😔 Lo siento, no tienes suficiente *Corazones ❤️* para comprar *${count}* Token`, m, rcanal);
  }
};
handler.help = ['rentar'];
handler.tags = ['grupo'];
