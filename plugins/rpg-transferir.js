...
async function handler(m, { conn, args, usedPrefix, command }) {
  if (confirmation[m.sender]) return conn.sendMessage(m.chat, {text: '*🚩 Aún hay fondos, en Transferencia, Aguarda un momento.*', mentions: [m.sender]}, {quoted: m});
...
`.trim();
  ...
  if (!who) return conn.sendMessage(m.chat, {text: '*🚩 Mensiona al usuario que desee hacerle la transferencia.*', mentions: [m.sender]}, {quoted: m});
